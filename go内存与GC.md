...
> 💡 **面试金句**：mcache 无锁是 Go 内存分配高性能的核心，99% 的小对象分配在此无锁完成，是 TCMalloc 三级缓存设计的精髓。
...
|----------|------|------------|----------|
| **内部碎片** | 分配的块 > 实际需要（块内浪费） | 请求 17B → 分配 24B，浪费 7B | 可控（单次最坏约 30%，平均约 12.5%） |
...
**查看工具**：
```bash
# 安装并使用 fieldalignment（golang.org/x/tools 独立工具，非内置 go vet）
...
MESI 是多核 CPU 的缓存一致性协议，保证多个核心看到的同一内存数据是一致的。
...
内存的"栈区"和"堆区"并非先有名字再决定用途——而是因为它们的**管理方式恰好像**对应的数据结构才被这样命名。

| 概念层面 | 本质 |
|---------|------|
| 数据结构的栈 | LIFO 抽象模型 |
| 内存的栈区 | 物理内存区域，管理方式符合 LIFO → 得名"栈" |
| 内存的堆区 | 物理内存区域，自由分配的内存池 → 得名"堆" |

**因果链**：

```
函数调用天然嵌套（A → B → C，C 返回，B 返回，A 返回）
        ↓ 符合 LIFO
用栈管理调用帧：分配/回收 = 移动 SP 指针，O(1)，极快，函数返回自动清理

数据生命周期超出函数作用域，无法用 LIFO 管理
        ↓
用堆：自由分配的内存池 + GC 管理，生命周期灵活
```

> **结论**：是内存区域的使用场景决定了管理方式（从而得名），不是反过来。Go 的逃逸分析（7.3 节）本质上就是编译器在两种管理方式之间自动做选择。
...
> 💡 **面试金句**：OS 线程栈固定 8MB + 内核态切换，goroutine 栈初始 2KB + 用户态切换——**4000 倍的内存差距 + 50 倍的切换速度差距**，这就是 Go 能开百万 goroutine 的根本原因。
...
> 💡 **面试比喻**：三色标记就是快递分拣——白色=未贴标签的包裹，灰色=贴了标签但未拆验，黑色=已拆完确认有主。BFS 顺序保证每个对象只扫描一次。
...
> 💡 **面试关键点**：Go 1.5 插入屏障满足强三色不变式，但栈上不开屏障，需要 STW 重扫描栈。Go 1.8 混合写屏障满足弱三色不变式，栈上对象在 GC 开始时全部标黑，**无需重扫描栈**，STW 降到亚毫秒级。
...
| **Go 1.6** | 插入屏障（仅堆） | 同上 | 2 次 | 优化并发清扫，STW 时间进一步缩短 |
| **Go 1.8+** | **混合写屏障** | 开始时栈标黑，新对象黑色 | 2 次（极短） | STW 仅做初始化/终止，< 500μs |

### 8.4 写屏障（Write Barrier）（核心⭐⭐⭐⭐）
...
> **注意**：截至 Go 1.26（2026-02），分代 GC 仍为试验特性，API 和行为可能变化。但这是 Go GC 最重要的演进方向。
...
与 JVM 对比的核心差异：
- **历史无分代**：传统 Go 统一三色标记，不区分年轻代/老年代；Go 1.24+ 已引入试验性分代 GC
...
| Go GC 为什么没有分代？ | 历史上维护代际信息开销得不偿失；Go 1.24+ 已引入试验性分代 GC（`GOEXPERIMENT=greenteagc`），是当前最重要的演进方向 |
| 什么是逃逸分析？ | 编译期决定变量分配在栈/堆的分析 |
| 如何查看逃逸分析？ | `go build -gcflags="-m"` |
| sync.Pool 的对象什么时候消失？ | **两轮 GC 后**（Go 1.13+ victim cache：第一轮降级，第二轮清除） |
| GOMEMLIMIT 是什么？ | 软内存上限，接近时触发更激进 GC，避免容器 OOM |
| mcache 是什么？ | **每个 P 私有的内存缓存**，小对象分配时完全无锁 |
| 为什么 goroutine 比线程轻？ | 初始栈 **2KB** vs 线程 **8MB**（Linux 默认）；用户态调度 vs 内核态切换 |
...
| make 和 new 的区别？ | `new(T)` 返回 `*T`（零值），任意类型；`make` 仅 slice/map/channel，返回初始化好的值，可直接使用 |
| 什么情况下变量会逃逸到堆？ | 返回局部变量指针、闭包引用外部变量、发送到 channel、赋值给 interface{}、编译期无法确定大小、大对象（>32KB） |
| 如何减少 GC 压力？ | `sync.Pool` 复用临时对象；预分配 slice/map 容量；减少指针字段（用值类型）；避免在循环中频繁分配小对象 |
| GOGC 参数是什么？ | 控制 GC 触发频率，默认 100（堆翻倍时触发）；调大→GC 频率↓吞吐↑；调小→GC 频率↑内存↓ |
